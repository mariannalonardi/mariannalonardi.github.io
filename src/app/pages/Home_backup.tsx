...
export default function Home() {
  ...
  // Commento temporaneamente per vedere sempre l'animazione durante lo sviluppo
  // useEffect(() => {
  ...
  const text = "Hi, I'm Marianna!";
  const letters = text.split("");

  // Funzione per calcolare la posizione di ogni lettera sul cerchio
  const getCirclePosition = (progress: number, radius: number) => {
    // progress va da 0 a 1 per un cerchio completo (360 gradi)
    const angle = progress * Math.PI * 2; // converti in radianti
    const x = Math.cos(angle - Math.PI / 2) * radius; // -PI/2 per iniziare dall'alto
    const y = Math.sin(angle - Math.PI / 2) * radius;
    const rotation = (progress * 360) + 90; // +90 per allineare il testo
    return { x, y, rotation };
  };
  ...
  return (
    <div className="min-h-screen">
      {/* Intro Animation Section - Prima schermata */}
      {showHeroAnimation && (
        <motion.section
          initial={{ opacity: 1 }}
          animate={{ opacity: 1 }}
        ...
        >
          {/* Stesso sfondo della hero section */}
          ...
          <div className="relative w-full h-full flex items-center justify-center z-10" style={{ perspective: "1500px" }}>
            {/* Foto progetti con effetto 3D parallax */}
            <div className="absolute inset-0 overflow-hidden">
              {[
                { img: moonymateImage, x: -450, y: -280, z: -200, rotation: -15, delay: 1.2 },
                { img: bob4Image, x: 450, y: -250, z: -300, rotation: 12, delay: 1.5 },
                { img: philips1Image, x: -500, y: 200, z: -250, rotation: 8, delay: 1.8 },
                { img: ubs5Image, x: 500, y: 250, z: -350, rotation: -10, delay: 2.1 },
                { img: provid1Image, x: -350, y: 30, z: -400, rotation: 18, delay: 2.4 },
                { img: green4Image, x: 350, y: -50, z: -280, rotation: -12, delay: 2.7 },
              ].map((project, index) => (
                <motion.div
                  key={index}
                  className="absolute"
                  ...
                  animate={{
                    x: [project.x * 0.3, project.x, project.x, project.x * 1.5],
                    y: [project.y * 0.3, project.y, project.y, project.y * 1.5],
                    z: [0, project.z, project.z, project.z - 200],
                    rotateY: [0, project.rotation, project.rotation, project.rotation * 1.5],
                    rotateX: [0, project.rotation * 0.5, project.rotation * 0.5, project.rotation * 0.8],
                ...
                >
                  <div
                    className="w-64 h-64 rounded-2xl overflow-hidden shadow-2xl border-2 border-white/20"
                    style={{
                      filter: "blur(0.5px)",
            ...
            {/* Ogni lettera con effetto prospettico profondo */}
            <div className="absolute" style={{ transformStyle: "preserve-3d" }}>
              {letters.map((letter, index) => {
                const letterDelay = index * 0.2;
                const animationDuration = 6;

                // Creo un arco a ponte con effetto 3D di profondità
                const bridgePoints = 80;
                const xValues = [];
                const yValues = [];
                const scaleValues = [];
                const opacityValues = [];
                ...
                // Genera punti lungo l'arco del ponte
                for (let i = 0; i <= bridgePoints; i++) {
                  const progress = i / bridgePoints;

                  // X va da -800 a +800
                  const x = -800 + (progress * 1600);

                  // Y segue una curva parabolica (forma del ponte)
                  const normalizedX = (progress - 0.5) * 2;
                  const y = -400 * (1 - normalizedX * normalizedX);

                  // Scale: parte normale, poi diventa più piccola verso la fine (effetto prospettico più marcato)
                  const scale = progress < 0.6 ? 1 : 1 - ((progress - 0.6) / 0.4) * 0.9;

                  // Z per effetto 3D profondo: le lettere vanno molto indietro nella prospettiva
                  const z = progress < 0.6 ? 0 : -((progress - 0.6) / 0.4) * 1200;

                  // Blur che aumenta progressivamente verso la fine
                  const blur = progress < 0.6 ? 0 : ((progress - 0.6) / 0.4) * 15;
                  const filter = `blur(${blur}px)`;

                  // Opacity: fade in all'inizio, normale al centro, fade out marcato alla fine
                  const opacity = progress < 0.08 ? progress / 0.08 :
                                 progress > 0.8 ? Math.pow((1 - progress) / 0.2, 1.5) : 1;
                ...
                return (
                  <motion.span
                    ...
                    style={{
                      ...
                      top: "50%",
                      transformStyle: "preserve-3d",
                      textShadow: "0 10px 30px rgba(0,0,0,0.8), 0 0 60px rgba(59, 130, 246, 0.5)",
                    }}
                    animate={{
                      x: xValues,
                      y: yValues,
                    ...
                    onAnimationComplete={() => {
                      if (index === letters.length - 1) {
                        setTimeout(() => setShowHeroAnimation(false), 500);
                      }
                    }}
                  >
                    {letter === " " ? "\u00A0" : letter}
